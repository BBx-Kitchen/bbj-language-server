...
export class BbjScopeProvider extends DefaultScopeProvider {
    ...
    protected superGetScope(context: ReferenceInfo) {
        const superScope = super.getScope(context)
        // console.error( `Scope size ${superScope.getAllElements().count()} for container ${context.container.$type} prop ${context.property}`)
        return superScope
    }
