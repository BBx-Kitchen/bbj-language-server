 ...
 *
 * Malformed free text (a bare `"10"` in a geometry field, an unterminated string, a bare numeric
...
<body>
  ...
  <fieldset id="geometry">
    ...
    <div class="grid">
      ...
      <div class="row"><label for="sysgui">SysGui expr</label><input type="text" id="sysgui"><div class="error" id="sysgui-error"></div></div>
...
<script nonce="${nonce}">
  ...
  window.addEventListener('message', (e) => {
    ...
    if (m.type === 'init') {
      ...
      const fieldErrors = {
        receiver: m.receiverError, sysgui: m.sysguiError, title: m.titleError,
        x: m.xError, y: m.yError, width: m.widthError, height: m.heightError,
      };
