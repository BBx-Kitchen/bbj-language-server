...
| 19 | CVS() composer compose and edit | 1. On a blank line, compose a new `CVS()` call selecting bits 1 and 4, add a `chars` value, insert<br>2. Edit the fixture's `1+4` call via its cue/lightbulb, change a bit, apply<br>3. While only bit 4 is checked, check the `chars` field's enabled state<br>4. Check the `mode%` call for a lightbulb | Step 1: the composed call is inserted with the chosen bits and chars. Step 2: the string argument is untouched and only the call's bits change. Step 3: the `chars` field is disabled (not hidden) while only bit 4 is checked. Step 4: the `mode%` call offers no lightbulb action | [ ] |
...
| 25 | CVS() dialog via Alt+Enter and the context menu | 1. Place the caret on a blank line, press Alt+Enter or use the editor context menu, compose a new `CVS()` call selecting bits 1 and 4, add a `chars` value, confirm<br>2. Place the caret on the fixture's `1+4` call, invoke the same entry, change a bit, apply<br>3. While only bit 4 is checked, check the `chars` field's enabled state<br>4. Place the caret on the `mode%` call and check for the reason notice | Step 1: the composed call is inserted with the chosen bits and chars. Step 2: the string argument is untouched and only the call's bits change. Step 3: the `chars` field is disabled (not hidden) while only bit 4 is checked. Step 4: no dialog opens; a reason notice explains why the call cannot be safely edited | [ ] |
