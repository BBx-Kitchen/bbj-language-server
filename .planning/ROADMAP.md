...
- [x] **Phase 68: Deliverable Documents** - Produce EASY-FIXES.md and MAJOR-REFACTORS.md with full coverage statement (completed 2026-08-19)
...
- [x] 60-04-PLAN.md — In-place ROADMAP/REQUIREMENTS corrections and staleness banners on the seven codebase maps
...
**Plans**: 7/7 plans executed — tracer + one per INVENTORY review unit, chained one per wave (D-01..D-04). Sole
...
- [x] 61-01-PLAN.md — wave 1, tracer: create the `61-COVERAGE.md` skeleton (grid, D-17 gate, 38 `n/a`
...
- [x] 61-02-PLAN.md — wave 2: sweep `RU-61-01` grammar & lexing (5 files / 1,340 LOC)
- [x] 61-03-PLAN.md — wave 3: sweep `RU-61-03` validation & BBjCPL diagnostics (8 files / 2,542 LOC)
- [x] 61-04-PLAN.md — wave 4: sweep `RU-61-02` scope, linking & type inference (8 files / 1,601 LOC)
- [x] 61-05-PLAN.md — wave 5: sweep `RU-61-04` LSP feature providers (11 files / 1,825 LOC)
- [x] 61-06-PLAN.md — wave 6: sweep `RU-61-05` server lifecycle, DI wiring & workspace (9 files / 1,433 LOC)
- [x] 61-07-PLAN.md — wave 7: sweep `RU-61-07` builtin catalogs (8 files / 3,752 LOC, mechanical per
...
- [x] 63-02-PLAN.md — Sweep `RU-63-01` run, compile & EM actions (11 files) across the 7 live dimensions, with concrete D1 records on the spawn and EM token surfaces, and triage the 3 inherited Phase 62 referrals
...
**Plans**: 3/3 plans executed — INVENTORY defines **no `RU-65-*` units**, so the grid, the cell gate and the file
...
issue-ready draft that Phase 69 files under ISSUE-01. Grouped by the kind of work each item needs
rather than by requirement number (D-14), chained one per wave since all three append to the single
...
- [x] 68-01-PLAN.md — Derivation spine: `derive-review-docs.mjs`, three-way `disposition:` selection with the 224/144/77/3 hard-fail gate, both deliverable documents on disk end-to-end, and the standing `check` gate
- [x] 68-02-PLAN.md — Record content: 77 easy rows lifted from `67-APPLY-SET.md`, 144 major blocks in INVENTORY frozen field order with `proposed_approach:` / `proposed_labels:` / empty `issue:`, plus both index tables
...
- [x] 68-07-PLAN.md — Close-out: FIX-04 discharged by name, the Phase 69 handoff and regeneration hazard, the Discrepancy Registers, and the D-12 write-boundary proof
...
**Plans**: 6/13 plans executed — the phase is inherently sequential (one approval gate, one append-target document,
...
- [x] 69-03-PLAN.md — Render filing-order rows 41-64 (24 `medium`) into `69-BODIES-03.md`, including both halves of the corpus's only `supersedes` pair, each body naming the other
- [x] 69-04-PLAN.md — Render filing-order rows 65-87 (23 `medium`) into `69-BODIES-04.md`, closing the 70-record `medium` block and glossing four review-internal references
- [x] 69-05-PLAN.md — Render filing-order rows 88-107 (20 `low`) into `69-BODIES-05.md`, with `P63-D3-005` proving the label rule against a prose `effort:` value
- [x] 69-06-PLAN.md — Render filing-order rows 108-125 (18 `low`) into `69-BODIES-06.md`
