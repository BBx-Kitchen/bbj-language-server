...
- 🚧 **v3.8 Test & Debt Cleanup** — Phases 54-56 (in progress)
...
### 🚧 v3.8 Test & Debt Cleanup (In Progress)
...
**Goal**: The test suite runs fully green with all 6 pre-existing failures fixed — either by correcting stale expectations to match current behavior or by fixing the underlying bugs the tests expose.
**Depends on**: Phase 53 (v3.7 complete)
**Requirements**: TEST-01, TEST-02, TEST-03, TEST-04, TEST-05, TEST-06
**Success Criteria** (what must be TRUE):
  1. `classes.test.ts` private and protected access error messages match the `(declared in ...)` format and pass without modification
  2. DEF FN parameters with `$` suffix complete correctly inside class methods (TEST-03 passes)
...
**Depends on**: Phase 54
**Requirements**: HARD-01, DEAD-01, DEAD-02
**Success Criteria** (what must be TRUE):
  1. All 9 previously commented-out `expectNoValidationErrors()` calls in `parser.test.ts` are uncommented and pass
...
**Depends on**: Phase 55
**Requirements**: FIX-01, FIX-02, FIX-03, FIX-04, TODO-01, TODO-02
**Success Criteria** (what must be TRUE):
  1. `bbj-linker.ts:74` receiver ref FIXME is resolved or replaced with an explanatory comment documenting why it is intentional
  2. `bbj-scope.ts:209` orphaned AST instances hack is resolved or replaced with a comment explaining the constraint
  3. `java-javadoc.ts:54` javadoc re-trigger FIXME is resolved (re-trigger implemented or deferred with rationale)
...
**Total:** 14 milestones shipped, 53 phases complete, 128 plans shipped. v3.8 in progress.

---

*Roadmap last updated: 2026-02-20 after v3.8 roadmap creation*
