...
must_haves:
  truths:
    ...
    - "Setting bbj.configPath in VS Code settings overrides the default config.bbx location derived from bbj.home"
  artifacts:
    - path: "bbj-vscode/package.json"
      provides: ".bbx in bbj language extensions; bbj.interop.host, bbj.interop.port, and bbj.configPath settings"
    ...
    - path: "bbj-vscode/src/language/bbj-ws-manager.ts"
      provides: "Uses configPath from settings to locate config.bbx instead of hardcoded bbjdir/cfg/config.bbx"
  key_links:
    - from: "bbj-vscode/src/extension.ts"
      to: "bbj-vscode/src/language/main.ts"
      via: "initializationOptions.interopHost, interopPort, and configPath"
      pattern: "interopHost|interopPort|configPath"
    ...
    - from: "bbj-vscode/src/language/bbj-ws-manager.ts"
      to: "config.bbx file"
      via: "configPath setting overrides default bbjdir/cfg/config.bbx location"
      pattern: "configPath"
---

<objective>
Merge .bbx into the BBj language in VS Code (same icon, diagnostics, completion, run commands), make java-interop host/port configurable, and add config.bbx path setting -- all with hot reload.

Purpose: Delivers IDE-01 (.bbx treated as BBj), CONF-01 (config.bbx path configurable), and CONF-02 (configurable interop host/port) for VS Code and the shared language server.
Output: .bbx files get full BBj treatment in VS Code; bbj.interop.host, bbj.interop.port, and bbj.configPath settings are available and take effect immediately.
</objective>
...
4. Add new configuration properties in `contributes.configuration.properties`:
   - `bbj.configPath`: type ["string", "null"], default null, description "Path to config.bbx file. When not set, defaults to {bbj.home}/cfg/config.bbx. Set this to use a custom config.bbx location for PREFIX resolution.", scope "window"
...
IMPORTANT: Do NOT change the `when` clauses — they currently work for the `bbx` language ID. Since .bbx will now be language `bbj`, the `resourceLangId == bbj` part of the conditions will match. The `|| resourceLangId == bbx` part becomes dead code but causes no harm.
  ...
  <verify>
Run `node -e "const p = require('./bbj-vscode/package.json'); console.log(p.contributes.languages.length); console.log(p.contributes.languages[0].extensions); console.log(Object.keys(p.contributes.configuration.properties).filter(k => k.includes('interop') || k.includes('configPath')))"` and confirm:
- Only 1 language entry
- Extensions include .bbx
- interop.host, interop.port, and configPath properties exist
  </verify>
  <done>.bbx is part of the BBj language in VS Code (same icon, same grammar). bbj.interop.host, bbj.interop.port, and bbj.configPath are configurable settings.</done>
</task>
...
1. In the `onInitialize` handler where `params.initializationOptions` is read, also extract `interopHost`, `interopPort`, and `configPath`:
   ```
...
3. Add a private field `private configPath = "";` to the class.
4. In the `initializeWorkspace()` method, update the config.bbx resolution logic. Currently it searches `{bbjdir}/cfg/config.bbx`. Change to:
   - If `this.configPath` is set and non-empty, use it directly as the path to config.bbx (read it as a file URI).
   - If `this.configPath` is not set, fall back to the existing behavior of searching `{bbjdir}/cfg/config.bbx`.
   This ensures the `bbj.configPath` setting overrides the default derived location.

**main.ts changes:**
In the `onDidChangeConfiguration` handler, after clearing cache and before reloading classpath, extract the new interop and configPath settings from the change object and update accordingly:
```typescript
...
```
Add a public `setConfigPath(path: string)` method to `BBjWorkspaceManager` that updates `this.configPath`.
Add the same interop logic to the `bbj/refreshJavaClasses` handler (extract settings from wsManager, apply to javaInterop before reconnecting).
...
5. Grep for `configPath` in bbj-ws-manager.ts, main.ts, extension.ts — should appear in all three
  </verify>
  <done>
...
4. package.json has bbj.interop.host, bbj.interop.port, and bbj.configPath settings
5. java-interop.ts uses configurable host/port, no hardcoded DEFAULT_PORT
6. initializationOptions includes interopHost, interopPort, and configPath
7. onDidChangeConfiguration extracts and applies interop settings and configPath
8. bbj-ws-manager.ts uses configPath to locate config.bbx when set, falls back to bbjdir/cfg/config.bbx when not
</verification>

<success_criteria>
- .bbx files in VS Code are recognized as BBj language with BBj icon and full language intelligence
- bbj.interop.host, bbj.interop.port, and bbj.configPath are visible in VS Code settings
- Language server reads interop host/port and configPath from initialization options
- Changing interop settings triggers cache clear and reconnection with new host/port
- Setting bbj.configPath overrides the default config.bbx location
- Default behavior (no settings configured) connects to localhost:5008 and uses bbjdir/cfg/config.bbx
</success_criteria>
