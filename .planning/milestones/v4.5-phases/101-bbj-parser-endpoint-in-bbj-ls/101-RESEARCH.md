...
|------------|-----------|----------|
| Parsing `doJSONSerialization()`'s JSON string with Gson | The lower-level `BBjProgram.readProgram(...)` entry point directly | `grammar-info.md` explicitly discourages this as coupling the language server to BBj internals that `ParserServiceAPI` deliberately hides; no reason to take it on for this phase |
