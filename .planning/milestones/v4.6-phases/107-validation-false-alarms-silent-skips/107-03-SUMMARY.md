...
key-decisions:
  - "A class-reference member used as the receiver of a further member access (e.g. Tree.Kind.CLASS) is exempted from the check even when no method/field matches, because java-interop's JavaClass model carries no nested-class membership data at all -- found by the whole-suite run flagging a real example file under this repo's own examples/ tree, fixed before committing Task 2"
