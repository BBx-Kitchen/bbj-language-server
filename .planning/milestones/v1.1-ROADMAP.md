...
Bring IntelliJ plugin to feature parity with VSCode for icons and run commands, fix all known v1.0 issues, and restore structure view.
...
|-------|----------------|--------|-----------|
| 7. Brand Icons | 1/1 | Complete | 2026-02-01 |
| 8. Run Commands | 2/2 | Complete | 2026-02-01 |
| 9. Structure View | 1/1 | Complete | 2026-02-02 |
| 10. Bug Fixes & Polish | 2/2 | Complete | 2026-02-02 |
...
- viewBox 0 0 3000 3000 with translate/scale for B-logo SVGs (preserves original path coordinates)
- Stroke-based rendering for Tabler icons (IntelliJ renders strokes correctly)
- BbxConfigFileType uses BbjLanguage.INSTANCE (shares language features for .bbx)
- Icon colors: #6E6E6E light, #AFB1B3 dark (IntelliJ standard)
- Project root as working directory for run commands (differs from VSCode's file directory)
- Auto-save defaults to true (matches VSCode behavior)
- web.bbj path via PluginManagerCore (robust plugin path discovery)
- Separate BUI/DWC action classes (clearer action registry)
- Three separate toolbar buttons (visible rather than dropdown)
- Single XML extension point for Structure View (LSP4IJ handles mapping)
- LINE_COMMENT_AT_FIRST_COLUMN = true for REM placement
- Distinct bracket token types for PairedBraceMatcher
- LSPHoverFeature.isSupported() = false to suppress hover placeholder
- 30-second grace period with FileEditorManagerListener
- Platform icons via AllIcons.Nodes for completion (native IntelliJ look)
- ARM64 detection on all platforms (not just macOS)

**Issues Resolved:**

- All 7 carried-forward v1.0 issues (FIX-01 through FIX-07)
- Completion icon pipeline wiring (discovered LSPCompletionFeature API, wired via setCompletionFeature)

**Issues Deferred:**

- Structure View symbol kind differentiation (LabelDecl/VariableDecl/FieldDecl all map to SymbolKind.Field) — language server issue
...
- Grace period runtime behavior not yet validated by human in running IDE
