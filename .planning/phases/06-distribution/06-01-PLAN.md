...
must_haves:
  truths:
    ...
    - "After installing from ZIP, language server starts and finds its bundled main.cjs without errors"
...
1. **Replace** the existing `patchPluginXml {}` block (inside the `tasks {}` section) with the new `pluginConfiguration {}` block inside `intellijPlatform {}`. This is a deliberate migration: both APIs are valid in IntelliJ Platform Gradle Plugin 2.11.0, but `pluginConfiguration {}` is the recommended approach and subsumes `patchPluginXml {}`. Add `pluginConfiguration` inside `intellijPlatform {}` with:
...
3. Remove the existing `patchPluginXml {}` block from inside the `tasks {}` section. It is now fully replaced by `pluginConfiguration {}` above — sinceBuild/untilBuild are now set in `pluginConfiguration.ideaVersion`, and description is set in `pluginConfiguration.description`.
