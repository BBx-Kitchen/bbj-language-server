...
**Public methods (async architecture — download never blocks the IDE):**

1. `@Nullable Path getCachedNodePath()` (synchronous, fast):
...
2. `void downloadNodeAsync(@NotNull Project project, @Nullable Runnable onComplete)`:
...
3. `private static String getPlatformName()`:
   - `SystemInfo.isMac` → "darwin"
   - `SystemInfo.isLinux` → "linux"
   - `SystemInfo.isWindows` → "win"

4. `private static String getArchitecture()`:
   - macOS: `SystemInfo.isAarch64` → "arm64", else "x64"
   - Others: `SystemInfo.is64Bit` → "x64" (don't support 32-bit, it's 2026)

5. `private static Path getNodeDataDirectory()`:
...
This gives users two paths: configure an existing Node.js in settings, or let the plugin download one automatically.
  ...
  <verify>
`./gradlew compileJava` succeeds. Review BbjLanguageServer.resolveNodePath() includes the download cache check. Review BbjMissingNodeNotificationProvider has "Download Node.js" action. Verify that the onComplete callback in the "Download Node.js" action calls `EditorNotifications.getInstance(project).updateAllNotifications()` so that opening a new BBj file after download does NOT show the missing-node banner.
  </verify>
  <done>
Language server startup chain checks downloaded Node.js cache. Missing-Node banner offers one-click download. After download completes, banner refreshes (via EditorNotifications.updateAllNotifications) and LS can start using downloaded Node.js. Opening a new BBj file after download does not show the missing-node banner.
  </done>
</task>
