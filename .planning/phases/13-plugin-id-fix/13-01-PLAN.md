...
<task type="auto">
  ...
  <action>
    ...
    3. Grep the entire bbj-intellij/src directory for `PluginId.getId("com.basis.bbj.intellij")` — there should be ZERO matches. Note: "com.basis.bbj.intellij" still legitimately appears in Java package declarations and import statements; only the PluginId.getId() calls needed to change.
  ...
  <verify>
    1. `grep -r 'PluginId.getId("com.basis.bbj.intellij")' bbj-intellij/src/` returns zero matches
...
<verification>
1. No PluginId.getId() calls referencing old plugin ID "com.basis.bbj.intellij" in any Java source file
