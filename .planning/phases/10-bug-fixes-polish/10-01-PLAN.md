...
must_haves:
  truths:
    ...
    - "Commenting an already-commented line stacks REM: 'REM code' becomes 'REM REM code'"
    - "Uncommenting strips exactly one REM from the start: 'REM REM code' becomes 'REM code'"
...
**Why this matches the CONTEXT.md stacking requirements:**

IntelliJ's `CommentByLineCommentHandler` (the built-in handler for Cmd+/) works as follows when `LINE_COMMENT_AT_FIRST_COLUMN = true` and line comment prefix is `"REM "`:

- **Commenting:** Prepends the literal prefix `"REM "` at column 0 of each selected line. It does NOT check whether the line is already commented. This means:
  - `    code here` becomes `REM     code here` (prefix at col 0, indentation preserved after it)
  - `REM original` becomes `REM REM original` (stacked -- second REM prepended)
  - Empty lines get `REM ` added (prefix on blank line)

- **Uncommenting:** The handler checks if each line starts with the prefix `"REM "` (or `"REM"` at line end). If it does, it strips exactly the FIRST occurrence of that prefix from the beginning of the line. This means:
  - `REM REM original` becomes `REM original` (one layer stripped)
  - `REM     code here` becomes `    code here` (back to original indentation)

- **Mixed selections:** When a selection contains both commented and uncommented lines, IntelliJ's default behavior with `LINE_COMMENT_AT_FIRST_COLUMN = true` is to COMMENT ALL lines (add prefix), not toggle individually. This stacks REM on already-commented lines.

This is exactly the behavior specified in CONTEXT.md decisions:
- REM at column 0: YES (LINE_COMMENT_AT_FIRST_COLUMN = true)
- Stack REM on already-commented lines: YES (handler always prepends, doesn't skip commented lines)
- Strip exactly one REM when uncommenting: YES (handler strips one prefix occurrence)
- Comment empty lines: YES (handler adds prefix to all selected lines)
...
- When uncommenting: strip exactly one `REM ` (or `REM` if no space follows)
  ...
  <verify>
Run `./gradlew build` from bbj-intellij/ directory -- build succeeds with no compilation errors. Visually verify: BbjCommenter returns "REM " prefix, BbjLanguageCodeStyleSettingsProvider sets LINE_COMMENT_AT_FIRST_COLUMN = true. After building, verify stacking behavior by creating a test BBj file in the IDE and:
1. Type `PRINT "hello"` on line 1 and `REM existing comment` on line 2
2. Select both lines, press Cmd+/ -- both lines should get `REM ` prepended: `REM PRINT "hello"` and `REM REM existing comment`
3. With both lines still selected, press Cmd+/ again -- should strip one REM from each: back to `PRINT "hello"` and `REM existing comment`
If manual testing is not possible in this execution context, confirm that (a) BbjLanguageCodeStyleSettingsProvider.getDefaultCommonSettings() returns settings with LINE_COMMENT_AT_FIRST_COLUMN = true, and (b) BbjCommenter.getLineCommentPrefix() returns "REM ".
  </verify>
  <done>
BbjCommenter returns "REM " prefix. BbjLanguageCodeStyleSettingsProvider forces LINE_COMMENT_AT_FIRST_COLUMN = true for BBj language. Both registered in plugin.xml. Comment stacking (REM REM) and single-layer uncommenting are guaranteed by IntelliJ's CommentByLineCommentHandler when LINE_COMMENT_AT_FIRST_COLUMN = true. Build compiles cleanly.
  </done>
</task>
...
4. Comment stacking verified: LINE_COMMENT_AT_FIRST_COLUMN = true causes IntelliJ to always prepend prefix (stacking) and strip one prefix on uncomment
5. BbjPairedBraceMatcher defines pairs for (), [], {}
6. BbjWordLexer emits bracket-specific token types
7. plugin.xml has lang.braceMatcher and langCodeStyleSettingsProvider registrations
8. BbjLanguageServerFactory either suppresses hover or has TODO comment
</verification>

<success_criteria>
- Build compiles with zero errors
- FIX-01: BbjCommenter + CodeStyle settings ensure REM at column 0 with correct stacking/unstacking behavior
