...
Run: `grep -n "super.createReferenceCompletionItem.*_refInfo.*_context" bbj-vscode/src/language/bbj-completion-provider.ts`
Expected: One match confirming super call forwards all 3 parameters (nodeDescription, _refInfo, _context)
...
4. Deprecated API audit: Grep across all files in bbj-vscode/src/language/ for any of these known removed APIs:
...
Run: `grep -rn "findDeclaration\|getLocalScopes\|getExportedElements" bbj-vscode/src/language/`
...
2. **Super call wiring**: `grep -c "super.createReferenceCompletionItem.*_refInfo.*_context" bbj-vscode/src/language/bbj-completion-provider.ts` returns 1
3. **Linker type guard**: `grep -c "isReference" bbj-vscode/src/language/bbj-linker.ts` returns at least 2 (import + usage)
4. **prepareLangiumParser unchanged**: `grep "prepareLangiumParser" bbj-vscode/src/language/bbj-module.ts` shows import and usage
5. **No deprecated APIs**: `grep -rn "findDeclaration\|getLocalScopes\|getExportedElements" bbj-vscode/src/language/` returns empty
6. **No pre-v3 imports**: No imports from removed langium subpaths
</verification>
