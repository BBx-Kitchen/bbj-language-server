...
must_haves:
  truths:
    - "Running 'Login to Enterprise Manager' action in IntelliJ prompts for credentials, launches em-login.bbj, and stores the returned token securely"
    - "BUI and DWC run actions in IntelliJ use the stored token instead of plaintext username/password"
    - "If no token is stored when a BUI/DWC run is triggered, the user is auto-prompted to log in via dialog (not just an error message)"
  ...
  key_links:
    ...
    - from: "BbjRunBuiAction.java"
      to: "BbjEMLoginAction.java"
      via: "performLogin() static method for auto-prompt login when no token stored"
---
...
An IntelliJ action that prompts for credentials, launches em-login.bbj, and stores the token.
IMPORTANT: The login logic MUST be in a static `performLogin(@Nullable Project project)` method that returns boolean, so BbjRunBuiAction/BbjRunDwcAction can call it for auto-prompt. The `actionPerformed()` method simply delegates to `performLogin(e.getProject())`.
...
2. REMOVE `emUsername` and `emPassword` fields from State class. These are replaced by JWT stored in PasswordSafe.
   NOTE: Removing fields from State won't cause errors for users with existing settings — IntelliJ's XML deserialization silently ignores unknown fields and uses defaults for missing fields.
...
**IMPORTANT: Extract login logic to a shared helper method.**

First, create a static helper method in BbjEMLoginAction (or a separate utility class) that performs the login flow programmatically without requiring an AnActionEvent. This is needed because the run actions must be able to trigger login as part of auto-prompt:

Add to BbjEMLoginAction:
```java
/**
...
public static boolean performLogin(@Nullable Project project) {
    ...
    // Prompt for credentials
    String username = Messages.showInputDialog(
        "Enter EM username:", "Enterprise Manager Login", null, "admin", null
    );
    if (username == null || username.isEmpty()) return false;

    String password = Messages.showPasswordDialog(
        "Enter EM password:", "Enterprise Manager Login"
    );
    if (password == null) return false;

    // Find em-login.bbj and launch (same logic as actionPerformed)
    // ... [reuse the BBj launch logic from actionPerformed]
    // On success: BbjEMTokenStore.storeToken(stdout); return true;
    // On failure: show error dialog; return false;
}
```

Refactor `actionPerformed()` to call `performLogin(e.getProject())`.

**BbjRunBuiAction.java changes:**
Replace the username/password credential retrieval with token retrieval AND auto-prompt login per locked decision ("auto-prompt on first EM access if not logged in"):
1. Remove:
   ```java
   ...
   ```
2. Add token retrieval with auto-prompt login:
   ```java
   String token = BbjEMTokenStore.getToken();
   if (token == null || token.isEmpty()) {
       // Auto-prompt login per locked decision
...
Apply the exact same auto-prompt pattern as BbjRunBuiAction (token retrieval with `BbjEMLoginAction.performLogin()` fallback).

Add the necessary imports to both run action files:
```java
import com.basis.bbj.intellij.actions.BbjEMTokenStore;
import com.basis.bbj.intellij.actions.BbjEMLoginAction;
```
  </action>
  <verify>
...
<verification>
1. BbjEMTokenStore.java uses PasswordSafe for token storage
2. BbjEMLoginAction.java has static `performLogin()` method and launches em-login.bbj
...
7. BbjRunBuiAction and BbjRunDwcAction call BbjEMLoginAction.performLogin() when no token (auto-prompt, NOT just error)
8. Run command lines pass token as param 8 to web.bbj
</verification>

<success_criteria>
- "Login to Enterprise Manager" action appears in IntelliJ Tools menu
- Login prompts for credentials, launches em-login.bbj, stores token in PasswordSafe
- BUI/DWC run actions retrieve token from PasswordSafe
- If no token stored, BUI/DWC run actions auto-prompt login dialog (not just error)
