...
must_haves:
  ...
  key_links:
    ...
    - from: "bbj-vscode/src/extension.ts"
      to: "bbj-vscode/src/language/main.ts"
      via: "synchronize.configurationSection: 'bbj' triggers workspace/didChangeConfiguration"
      pattern: "configurationSection.*bbj"
    - from: "bbj-vscode/src/language/main.ts"
      to: "bbj-vscode/src/language/java-interop.ts"
      via: "javaInterop.clearCache() call in both manual refresh and config change handler"
      pattern: "clearCache"
...
**Part C: Listen for workspace configuration changes to trigger automatic refresh**

Per user decision: "Re-scan classpath when user changes classpath/config settings." Add a `workspace/didChangeConfiguration` handler in `main.ts` alongside the custom request handler. This handler should detect changes to BBj classpath/home settings and automatically trigger the same refresh flow.

Currently, `bbj-ws-manager.ts` reads settings during `initializeWorkspace` from `initializationOptions` (lines 31-52) but never listens for configuration changes after startup. There is no existing `onDidChangeConfiguration` handler anywhere in the server code.

Add this handler in `main.ts` after the `RefreshJavaClassesRequest` handler and before `startLanguageServer`:

```typescript
// Listen for configuration changes to BBj settings (classpath, home)
// Per user decision: re-scan classpath when user changes classpath/config settings
connection.onDidChangeConfiguration(async (_change) => {
    try {
        ...
        // Check if classpath or home changed
        ...
        if (!classpathChanged && !homeChanged) {
            // No BBj-relevant settings changed, skip refresh
            return;
        }

        console.log('BBj settings changed, refreshing Java classes...');

        // Perform the same refresh flow as the manual command
        javaInterop.clearCache();

        // Determine classpath to load
...
**IMPORTANT:** The VS Code client must also be configured to notify the server of configuration changes. In `extension.ts`, the `startLanguageClient` function must include `'bbj'` in the `synchronize.configurationSection` option of `LanguageClientOptions`. If this is not already present, add it:
...
const clientOptions: LanguageClientOptions = {
    ...
    synchronize: {
        ...
        configurationSection: 'bbj'  // <-- Ensure this is set
    },
    initializationOptions: { ... }
};
```

This ensures VS Code sends `workspace/didChangeConfiguration` notifications to the server when any `bbj.*` setting changes (e.g., `bbj.classpath`, `bbj.home`). Without this, the server would never receive the notification.
  </action>
  <verify>
...
5. Review: `onDidChangeConfiguration` handler in main.ts detects BBj settings changes and triggers refresh
6. Review: extension.ts includes `configurationSection: 'bbj'` in synchronize options
  </verify>
  <done>
JavaInteropService has clearCache() method. main.ts has bbj/refreshJavaClasses request handler that performs full cache invalidation, classpath reload, document re-validation, and user notification. Configuration change listener automatically triggers refresh when BBj classpath/home settings change.
  </done>
</task>
...
- Changing BBj classpath or home settings in VS Code triggers automatic classpath re-scan (via onDidChangeConfiguration in main.ts + configurationSection in extension.ts)
