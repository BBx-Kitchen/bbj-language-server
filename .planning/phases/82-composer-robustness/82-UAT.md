...
number: 1
name: Lightbulb popup no longer throws and the Intentions settings page renders the shipped resources
expected: |
  With the rebuilt plugin installed (bbj-vscode `npm run build`, then bbj-intellij `./gradlew buildPlugin`;
  confirm the installed BBj build is the local one, not a Marketplace auto-update), put the caret on a
  `MSGBOX(...)`, `addWindow(...)` and `addChildWindow(...)` call in turn, press Alt+Enter and arrow onto
  the "Configure … with the BBj visual composer" entry so the preview pane computes. No error report
  appears and no `Intention Description Dir URL is null` exception is logged (check Help › Show Log if
  in doubt). Then open Settings › Editor › Intentions, expand the BBj category: each of the three
  entries shows a description and a before/after example instead of an empty panel.
