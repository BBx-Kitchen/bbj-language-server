...
<task type="auto">
  ...
  <action>
    ...
    4. In `InteropService.java` `loadClassInfo()` method:
       ...
       b. After extracting methods, set `mi.isStatic = Modifier.isStatic(m.getModifiers())` and `mi.isDeprecated = m.isAnnotationPresent(Deprecated.class)` for each method. **IMPORTANT:** Verify the method loop uses `clazz.getMethods()` (NOT `getDeclaredMethods()`). `getMethods()` returns all public methods including inherited ones from superclasses, which is required so that inherited static methods (e.g., AbstractList statics on ArrayList) appear in completion. The existing code already uses `getMethods()` — do NOT change it to `getDeclaredMethods()`.
