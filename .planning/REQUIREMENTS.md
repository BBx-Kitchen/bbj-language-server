...
- [ ] **DISC-05**: User hovering a `SETOPTS` literal, or an `IOR`/`AND` line against an OPTS-derived variable in BBj code, sees which options that line sets or clears, with AND masks shown as the logical cleared bits (#475, decode tier) — Plan 88-01 landed the absolute-literal shape only; the `IOR`/`AND` chain and single-call shapes land in Plan 88-02
...
| DISC-05 | Phase 88 | In Progress (88-01 landed the absolute shape; 88-02 pending) |
