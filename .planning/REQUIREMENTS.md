...
- [x] **BUILD-04**: The existing `src/test/` source set (7 JUnit 5 classes, stale claim in #569 corrected) is extended with regression coverage for the Node download/extract/cache pipeline and the EDT-responsiveness paths this milestone changes; `./gradlew test` runs them green (#569, residual)
- [x] **BUILD-05**: Every LSP4IJ `@ApiStatus.Experimental` coupling point across the seven `lsp/` and `ui/` files, plus the new compile request surface from PARITY-01, is covered by canary or source-guard regression tests that fail on a breaking LSP4IJ change (#544; closes #554 as a subset)
