...
- [x] **DISC-02**: User is offered the MSGBOX composer when the options argument is an expression; a sum of constant Java static fields or integer literals pre-fills the composer, any other expression opens it in compose-and-replace mode (#648)
- [x] **DISC-03**: User can compose a CVS() call visually in both IDEs from the documented bit operations (1, 2, 4, 8, 16, 32, 64, 128; applied in ascending order) with the version-gated `chars` parameter, and can edit an existing literal-mask CVS() call in place (#649)
...
| DISC-02 | Phase 89 | Complete |
| DISC-03 | Phase 89 | Complete |
